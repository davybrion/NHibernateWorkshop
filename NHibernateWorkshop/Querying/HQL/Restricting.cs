...
namespace NHibernateWorkshop.Querying.HQL
{
    ...
    public class Restricting : AutoRollbackFixture
    {
        ...
        public void where_properties_are_equal()
        {
            ...
            var products = Session.CreateQuery("from Product p where p.UnitsInStock = p.UnitsOnOrder").List<Product>();
        ...
        public void where_one_property_is_greater_than_the_other()
        {
            ...
            var products = Session.CreateQuery("from Product p where p.UnitsOnOrder > p.UnitsInStock").List<Product>();
        ...
        public void where_one_property_has_a_value_between_a_given_range()
        {
            ...
            var products = Session.CreateQuery("from Product p where p.UnitsInStock between :min and :max")
                .SetParameter("min", 5)
                .SetParameter("max", 10)
                .List<Product>();
        ...
        public void where_one_property_has_a_value_in_a_given_set()
        {
            ...
            var products = Session.CreateQuery("from Product p where p.UnitsInStock in (:collection)")
                .SetParameterList("collection", stockLevels)
                .List<Product>();
        ...
        {
            var products = Session.CreateQuery("from Product p where p.Name like :value")
                .SetParameter("value", "%roduc%")
                .List<Product>();
        ...
        {
            var products = Session.CreateQuery("from Product p where p.ReorderLevel is null").List<Product>();
        ...
        {
            var products = Session.CreateQuery("from Product p where p.ReorderLevel is not null").List<Product>();
        ...
        {
            var products = Session.CreateQuery("from Product p where size(p.Sources) = 0").List<Product>();
        ...
        public void two_restrictions_with_and()
        {
            ...
            var products = Session.CreateQuery("from Product p where size(p.Sources) > 0 and p.UnitsInStock is not null").List<Product>();
        ...
        {
            var products = Session.CreateQuery("from Product p where size(p.Sources) > 0 or p.UnitsInStock is null").List<Product>();
        ...
        public void more_than_two_restrictions_with_and()
        {
            ...
            var products =
                Session.CreateQuery(
                    @"from Product p where size(p.Sources) > 0 
                      and p.UnitsInStock is not null and p.ReorderLevel is null")
                    .List<Product>();
        ...
        public void more_than_two_restrictions_with_or()
        {
            var products =
                Session.CreateQuery(
                    @"from Product p where size(p.Sources) > 0 
                      or p.UnitsInStock is not null or p.ReorderLevel is null")
                    .List<Product>();

            Assert.IsTrue(products.Contains(_product1));
            Assert.IsTrue(products.Contains(_product2));
            Assert.IsTrue(products.Contains(_product3));
            products.Each(p => Assert.IsTrue(p.Sources.Count() > 0 || p.UnitsInStock != null || p.ReorderLevel == null));
        }
    }
}